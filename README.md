...
We have already initialized the perceptron weights `self.w` to be a **1 × dimensions**
 parameter node. The provided code will include a bias feature inside `point` when needed,
so you will not need a separate parameter for the bias.

Your tasks are to:
1. Implement the `run(self, point)` method.
  This should compute the dot product of the stored weight vector and the given input,
  returning an nn.DotProduct object.

2. Implement `get_prediction(self, point)`, which should return 1 if the dot product is non-negative or -1
...
# Question 2 (1 point): Non-linear Regression
...
* `nn.AddBias` adds a bias vector to each feature vector.
  * Usage: `nn.AddBias(bias, features)` accepts bias of shape `1 × num_features` and features of shape batch_size × num_features,
